...
export const FilmsToolbar: React.FC = () => {
  ...
  return (
    <GridToolbarContainer>
      <Button startIcon={<AddIcon />} onClick={() => router.push(paths.filmCreate({}))}>
        Создать фильм
      </Button>
    </GridToolbarContainer>
  );
...
> = ({ onDelete, ...props }) => {
  ...
  const filmColumns: GridColumns = useMemo(
    () => [
      ...
      { field: 'description_short', headerName: 'Краткое описание', width: 150 },
  ...
  return <DataGrid columns={filmColumns} checkboxSelection pagination {...props} />;
};
